...
function viewKanban() {
  const cols = ['em_analise', 'aprovado', 'aguardando_aprovacao', 'em_producao', 'pronto_envio', 'enviado'];
...
async function saveProduct(e, id) {
  ...
  const body = { name: f.name.value, category: f.category.value, icon: f.icon.value, badge: f.badge.value, tagline: f.tagline.value, desc: f.desc.value, quantities, offerPct: Math.min(90, Math.max(0, Number(f.offerPct.value) || 0)), offerEnds: f.offerEnds.value || '' };
...
function editCoupon(id) {
  ...
  <div style="margin-top:8px"><label style="font-weight:700"><input type="checkbox" name="firstOnly" ${c.firstOnly ? 'checked' : ''} style="width:18px;height:18px;vertical-align:-3px"> Só na primeira compra</label></div>
